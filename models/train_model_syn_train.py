...
undisp_cube, mask, spectras = hlp.create_bs_data(desired_channels=21,kernel=kernel, cube_dir ='20230629_mask_analysis_800us_talbot_0', fts_dir = '20230628_2gratings_mask_data_10000us_talbot_0',interp_type='average',device=device) #here we'll work with synthetic data. 
...
model = FourierDenoiser(mask=mask,kernel=kernel,CoordGate=False, trainable_kernel=True).to(device)
...
torch.save(model.state_dict(), 'syn_train_unet')
